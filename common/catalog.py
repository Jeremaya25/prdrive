#!/usr/bin/env python3
"""
catalog.py — El catálogo global de parejas, que vive en el NAS.

`sync_config.toml` dice qué sincroniza ESTE pen; el catálogo
(`synology:/PJ/Perepen-catalog/pairs.toml`) dice qué parejas existen, y es el
mismo fichero para todos los dispositivos. El alta y la baja de una pareja
ocurren ahí primero; cada pen se limita a elegir cuáles de ellas usa y, si le
hace falta, a modificar su copia local dejándola marcada como divergente.

Tres cosas gobiernan este módulo:

  * **Leer nunca puede matar la ventana.** `load()` intenta el NAS, y si no hay
    red cae a la copia de `state/catalog.toml`. Sin copia devuelve `None` y un
    aviso, y la pantalla sigue abriéndose con lo que ya tiene el pen.
  * **Escribir es lo peligroso.** Este fichero gobierna borrados en todos los
    ...
    se niega si el remoto ha cambiado bajo sus pies y deja un `.bak` en el NAS.
...
# El mismo que tiene embebido perepen-install.py, que es quien lee el catálogo
# cuando todavía no hay pen. Se puede cambiar por dispositivo desde [defaults].
DEFAULT_CATALOG_PATH = "/PJ/Perepen-catalog/pairs.toml"
BAK_SUFFIX = ".bak"

# El catálogo son unos kilobytes, así que aquí no se espera por ancho de banda:
# se espera a un NAS que puede no estar. Y quien espera es la ventana de parejas,
# que se abre en cuanto se enchufa el pen. Con los valores de rclone por defecto
# (5 min de --timeout, 3 reintentos) una wifi mala congela la UI varios minutos;
# con estos, un NAS inalcanzable se resuelve en segundos y se cae a la copia.
...
def cache_toml() -> Path:
    ...
    quedaría apuntando al pen de verdad."""
...
class Catalog(NamedTuple):
    ...
    endpoint: str              # 'synology:/PJ/Perepen-catalog/pairs.toml'

    @property
    def editable(self) -> bool:
        """Solo se escribe sobre lo que se acaba de leer del NAS.

        Subir partiendo de la copia local sería escribir a ciegas encima de lo
        que otro pen haya hecho mientras tanto."""
...
def endpoint(raw_local: Mapping[str, Any] | None = None) -> str:
    """Dónde está el catálogo, según los [defaults] de este pen."""
...
def run(args: list[str]) -> subprocess.CompletedProcess:
    """rclone con el config y el cwd del pen.
...
def _write_cache(cat: Catalog) -> None:
    """Guardar la copia local. Que falle no es un error: el pen puede estar de
    solo lectura o haberse extraído a media frase."""
    try:
...
def pull(raw_local: Mapping[str, Any] | None = None) -> Catalog:
    """Leer el catálogo del NAS y cachearlo. ConfigError si no se puede."""
...
def load(raw_local: Mapping[str, Any] | None = None) -> tuple[Catalog | None, str | None]:
    ...
    return None, (f"No hay catálogo ni copia local, así que solo se puede trabajar "
                  f"con las parejas que ya tiene este pen.\n{motivo}")
...
def push(new_raw: Mapping[str, Any], base_text: str,
    ...
    if actual.stdout != base_text:
        raise ConfigError(
            "El catálogo ha cambiado en el NAS desde que lo leíste, así que no se "
            "ha escrito nada. Otro dispositivo lo ha tocado mientras tanto: cierra "
    ...
    tmpdir = Path(tempfile.mkdtemp(prefix="perepen-cat-"))
    try:
...
# ---------------------------------------------------------------------------
# Comparación con lo que tiene el pen
# ---------------------------------------------------------------------------
