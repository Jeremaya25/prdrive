...
| 15 (#50) | El paquete **VeraCrypt Portable** es el `.exe` del extractor con un bloque detrás: `VCINSTRT` · tamaño sin comprimir y comprimido (u32 BE, `mputLong`) · LZMA (5 bytes de propiedades + flujo **sin marca de fin**: `LzmaCompress` llama a `LzmaEncode` con `writeEndMark` 0) · `VCINSCRC` · CRC-32. Por fichero: nombre (u16 BE en `wchar_t` + UTF-16LE), CRC-32, longitud, contenido. Trae `VeraCrypt-x64.exe`, `-arm64.exe`, sus Format y Expander, los dos `.sys` con su `.cat`, el `.inf`, las licencias, `docs.zip` y `Languages.zip`; ningún `VeraCrypt.exe`. | `Setup/SelfExtract.c` (`MakeSelfExtractingPackage`, `SelfExtractInMemory`), `Setup/Setup.h` (`szCompressedFiles`, PORTABLE), `Common/lzma/LzmaLib.c` | `install/veracrypt_bin.py` lo abre sin ejecutarlo. Comprobado contra el `VeraCrypt Portable 1.26.24.exe` de Launchpad (40 413 480 bytes, SHA-256 `99c166a3…049a`): 16 ficheros, todos los CRC correctos. |
| 16 (#50) | `FindStringInFile()` busca **desde el final** la última aparición; `VCINSTRT` sale dos veces (la primera, en el código del extractor) y `VCINSCRC` una, porque en el código va ofuscado. `VerifyPackageIntegrity()`: CRC-32 del fichero hasta el marcador final incluido, con 0x130–0x1ff a cero (`WipeSignatureAreas`), contra el u32 de detrás; `SelfExtractInMemory()` exige que el bloque comprimido llegue justo al marcador. | `Setup/SelfExtract.c`, `Common/Dlgcode.c` | Las mismas tres comprobaciones, además del SHA-256 fijado. |
| 17 (#50) | En modo portátil el diálogo del traveler copia `VeraCrypt-x64.exe`, `-arm64.exe` y los dos drivers **con esos nombres**; desde una instalación MSI renombra `VeraCrypt.exe` → `VeraCrypt-x64.exe` y `veracrypt.sys` → `veracrypt-x64.sys`. | `Mount/Mount.c` (`TravelerDlgProc`) | Viajan los nombres del portable; la copia sin red de una instalación se renombra igual. |
| 18 (#50) | La copia elevada es la misma imagen: `InitApp` relanza lo que da `GetModuleFileNameW`. | `Common/Dlgcode.c` (`InitApp`, `LaunchElevatedProcess`) | `_esperar_copia_elevada()` y `:vc_pendiente` buscan el nombre del ejecutable lanzado (`VeraCrypt Format-x64.exe`, `VC_IMAGEN`). |
